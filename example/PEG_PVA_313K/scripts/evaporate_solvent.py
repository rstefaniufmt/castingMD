...
def resid_above(resname, zcut):
    return [r for r in u.select_atoms(f"resname {resname}").residues
            if r.center_of_mass(pbc=True)[2] > zcut]

rm_sol = resid_above("SOL", cut_sol)
rm_ace = resid_above("ACE", cut_ace)
...
def patch_top(top_in, top_out, nsol_out, nace_out):
    with open(top_in) as fin, open(top_out, "w") as fout:
        for line in fin:
            ...
            elif line.lstrip().startswith("ACE"):
                tokens = line.split()
...
nace_old = len(u.select_atoms("resname ACE").residues)
