...
cut_sol = 0.95 * plane_frac * Lz                 # limite para água (menor)
cut_ace = plane_frac * Lz           # limite menor p/ acido acetico
...
rm_ace = resid_above("ACOH", cut_ace)
...
def patch_top(top_in, top_out, nsol_out, nace_out):
    with open(top_in) as fin, open(top_out, "w") as fout:
        for line in fin:
            ...
            elif line.lstrip().startswith("ACOH"):
                tokens = line.split()
...
nace_old = len(u.select_atoms("resname ACOH").residues)
patch_top(in_top, out_top,
          nsol_old - len(rm_sol),
          nace_old - len(rm_ace))

print(f"Removidos: {len(rm_sol)} água; {len(rm_ace)} acOH")
