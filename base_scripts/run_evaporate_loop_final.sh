...
for ((i=0; i<CYCLES; i++)); do
    echo "==== Iniciando ciclo $i ====" | tee -a "$LOG_FILE"
    
    tag=$(printf "%d" ${i})
    
    if [[ -f "step${tag}_last.gro" ]]; then
        echo "Arquivo step${tag}_last.gro já existe. Pulando ciclo $i." | tee -a "$LOG_FILE"
        continue
    fi
    ...
    if [[ $i -ne 0 ]]; then
        ...
        if [[ ! -f $GRO ]]; then
         ...
         echo ">>> Calculando nova altura da caixa..." | tee -a "$LOG_FILE"
        ...
        if [ -z "$rlist_real" ]; then
          echo "[AVISO] Não foi possível encontrar ajuste de rlist no log. Usando valor padrão do TPR." | tee -a "$LOG_FILE"
        ...
        if [ "$zsafe" -eq 1 ]; then
            echo ">>> Altura Z calculada ($zmax nm) é muito pequena. Corrigindo para ${minZ} nm" | tee -a "$LOG_FILE"
            zmax=$minZ
            cp ${GRO} old_${GRO}
            
            # 5. Redefine caixa com nova altura
            echo ">>> Aplicando nova altura da caixa Z = ${zmax} nm" | tee -a "$LOG_FILE"
            ...
            echo "[INFO] Verificando consistência após ciclo $i" | tee -a "$LOG_FILE"
            n_atoms_gro=$(tail -n +3 ${GRO} | wc -l)
            n_sol_top=$(grep "^SOL" ${TOP} | awk '{print $2}')
            echo "  Átomos no .gro: $n_atoms_gro" | tee -a "$LOG_FILE"
            echo "  Moléculas de SOL no .top: $n_sol_top" | tee -a "$LOG_FILE"
        fi 
...
# 1. Preparar TPR
    ...
    # ------ Roda MD ------
    gmx mdrun -deffnm step${tag} -v || { echo "Erro no mdrun no ciclo $i" | tee -a "$LOG_FILE"; exit 1; }
   ...
   # 3. Extrair o último quadro
    ...
    rm -rf step${tag}.trr 
    # ------ Evaporação ------
    echo "Removendo solvente (plane_frac=$plane)..." | tee -a "$LOG_FILE"
    python3 evaporate.py step${tag}_last.gro ${TOP} step${tag}_post.gro step${tag}_post.top ${plane} || { echo "Erro na evaporação no ciclo $i" | tee -a "$LOG_FILE"; exit 1; }

    # ------ Atualiza para próximo ciclo ------
    GRO=step${tag}_post.gro
    TOP=step${tag}_post.top
    
    rlist_real=$(grep "rlist from" step${tag}.log | tail -1 | awk '{print $11}')
    buffer=0.3
    
    if [ -z "$rlist_real" ]; then
        echo "[AVISO] Não foi possível encontrar ajuste de rlist no log. Usando valor padrão do TPR." | tee -a "$LOG_FILE"
        rlist_real=$(gmx dump -s step${tag}.tpr | grep "rlist" | head -1 | awk '{print $3}')
    fi
    ...
    gmx mdrun -deffnm step${tag}_min -v || { echo "Erro no EM no ciclo $i" | tee -a "$LOG_FILE"; exit 1; }
    
    # --- novo nvt e mdp
    gmx grompp -f nvt_eq.mdp -c step${tag}_min.gro -r step${tag}_min.gro -p ${TOP} -o step${tag}_nvt.tpr -maxwarn 10
    gmx mdrun -deffnm step${tag}_nvt -v || { echo "Erro no NVT no ciclo $i" | tee -a "$LOG_FILE"; exit 1; }

    gmx grompp -f npt_eq.mdp -c step${tag}_nvt.gro -t step${tag}_nvt.cpt -r step${tag}_nvt.gro -p ${TOP} -o step${tag}_npt.tpr -maxwarn 10
    gmx mdrun -deffnm step${tag}_npt -v || { echo "Erro no NPT no ciclo $i" | tee -a "$LOG_FILE"; exit 1; }
...
echo "==== Simulação completa até ciclo $CYCLES, resfriando sistema... ====" | tee -a "$LOG_FILE"
...
gmx mdrun -deffnm filme_final -v || { echo "Erro no resfiamento" | tee -a "$LOG_FILE"; exit 1; }

echo "==== Simulação completa !! ====" | tee -a "$LOG_FILE"
